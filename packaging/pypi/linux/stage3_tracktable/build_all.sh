...
for PYTHON_IMPLEMENTATION in cp36 cp37 cp38 cp39 cp310; do
   docker build -t tracktable:${PYTHON_IMPLEMENTATION} -f Dockerfile.build_tracktable --build-arg PYTHON_IMPLEMENTATION=${PYTHON_IMPLEMENTATION} --build-arg NIGHTLY=${NIGHTLY} --build-arg CI_REGISTRY=${CI_REGISTRY} .
done
