...
unsigned int TurnArounds(trajectory_type &trajectory)
{
  ...
  trajectory_type::iterator itr1 = trajectory.begin();
  trajectory_type::iterator itr2 = itr1 + window;
  double diff;
  bool found = false;
  ...
  do {
    ...
    if (fabs(diff - 180.0) < 2.0) {  // 2.0 is the fudge factor for 180
      if (!found) {
        ++ctr;
      }
      found = true;
...
double StraightFraction(trajectory_type &trajectory)
{
  ...
  trajectory_type::iterator itr1 = trajectory.begin();
  trajectory_type::iterator itr2;

  do {
  // The 2.0 below is related to how different the straightness can be from 180
    itr2 = std::adjacent_find(itr1,trajectory.end(),
     !boost::bind(std::less<double>(),
     boost::bind(fabs,(boost::bind(TrajHeadingDifference,_1,_2))),2.0));
